...
const AREA = "Mill Hill";
const SLUG = "mill-hill";
const POSTCODE = "NW7";
const PHONE = "+447789471859";

const META_DESCRIPTION =
  "Looking for driving lessons in Mill Hill (NW7)? 360 Drive Academy offers DVSA-approved instructors, manual & automatic lessons, intensive courses and mock tests across Mill Hill. Book today.";

const INTRO_TEXT =
  "360 Drive Academy provides professional driving lessons in Mill Hill, NW7. Our fully qualified, DVSA-approved instructors live and teach locally — they know Mill Hill's roads, junctions and test routes inside out. Whether you’re a complete beginner or returning after a break, we’ll get you confident and test-ready.";

const BODY_TEXT =
  "From Mill Hill Broadway to the A1 and nearby A41 corridors, our lessons prepare students for the real driving situations they will meet on test day. We offer beginner lessons, intensive courses, Pass Plus, motorway tuition and mock tests — all in modern dual-control vehicles. Lessons are available 7 days a week including evenings.";

const SERVICES = [
  "Beginner driving lessons in Mill Hill, NW7",
  "Manual & automatic car options",
  "Intensive & crash course driving programmes",
  "Mock driving test on local Mill Hill routes",
...
const NEARBY_AREAS = [
  ...
  { name: "Burnt Oak", href: "/areas/burnt-oak" },
  { name: "Edgware", href: "/areas/edgware" },
  { name: "Mill Hill Broadway", href: "/areas/mill-hill-broadway" },
  { name: "Golders Green", href: "/areas/golders-green" },
  { name: "Barnet", href: "/areas/barnet" },
  { name: "Kingsbury", href: "/areas/kingsbury" },
];

// Google Maps embed — centred on Mill Hill (NW7)
const MAP_URL = "https://www.google.com/maps?q=Mill+Hill+NW7&output=embed";

const MillHillPage = () => {
  useEffect(() => {
    document.title = `Driving Lessons in Mill Hill NW7 | 360 Drive Academy`;
    ...
    setMeta("keywords", "driving lessons mill hill, driving instructor mill hill, automatic lessons mill hill, intensive course mill hill");
    setMeta("og:title", `Driving Lessons in Mill Hill NW7 | 360 Drive Academy`, "property");
